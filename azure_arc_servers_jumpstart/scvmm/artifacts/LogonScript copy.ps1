...
$Env:SCVMMIconDir = "C:\SCVMM\Icons"

Start-Transcript -Path $Env:SCVMMLogsDir\LogonScript.log

$cliDir = New-Item -Path "$Env:SCVMMDir\.cli\" -Name ".dataops" -ItemType Directory
...
$sourceFile = $sourceFile + "?" + $sas
azcopy cp --check-md5 FailIfDifferentOrMissing $sourceFile  "$Env:SCVMMLogsDir\installCAPI.log"
...
foreach ($cluster in $clusters) {
    Start-Job -Name SCVMM -ScriptBlock {
        $cluster = $using:cluster
        $context = $cluster.context
        Start-Transcript -Path "$Env:SCVMMLogsDir\DataController-$context.log"
        ...
        Copy-Item "$Env:SCVMMDir\dataController.parameters.json" -Destination "$Env:SCVMMDir\dataController-$context-stage.parameters.json"

        $dataControllerParams = "$Env:SCVMMDir\dataController-$context-stage.parameters.json"
    ...
    (Get-Content -Path $dataControllerParams) -replace 'logAnalyticsPrimaryKey-stage', $workspaceKey | Set-Content -Path $dataControllerParams
        ...
        az deployment group create --resource-group $Env:resourceGroup --name $cluster.dataController --template-file "$Env:SCVMMDir\dataController.json" --parameters "$Env:SCVMMDir\dataController-$context-stage.parameters.json"
        Write-Host "`n"
        ...
        Write-Host "`n"
        Remove-Item "$Env:SCVMMDir\dataController-$context-stage.parameters.json" -Force
...
while ($(Get-Job -Name SCVMM).State -eq 'Running') {
    Receive-Job -Name SCVMM -WarningAction SilentlyContinue
    Start-Sleep -Seconds 5
}

Get-Job -name SCVMM | Remove-Job
write-host "Successfully deployed Azure Arc Data Controllers"

Write-Header "Deploying SQLMI"
# Deploy SQL MI data services
& "$Env:SCVMMDir\DeploySQLMIADAuth.ps1"

Start-Transcript -Path $Env:SCVMMLogsDir\LogonScript.log -Append
...
# Deploy App
& "$Env:SCVMMDir\DataOpsAppScript.ps1"
...
# Changing to Jumpstart SCVMM wallpaper
$code = @' 
